...
let lastCode = '';
let matchCount = 0;
let pendingEditJan = null; // For View Mode Dialog
...
const dom = {
  ...
  dlgShelf: document.getElementById('dlg-shelf'),
  dlgKyakuchu: document.getElementById('dlg-kyakuchu'),
  dlgPrefab: document.getElementById('dlg-prefab'),
  btnDlgSave: document.getElementById('btn-dialog-save'),
  btnDlgCancel: document.getElementById('btn-dialog-cancel'),
};
...
const app = {
  ...
  setupEvents() {
    ...
    // Dialog Events
    dom.btnDlgCancel.onclick = () => dom.dialog.classList.add('hidden');
    dom.btnDlgSave.onclick = () => this.saveViewEdit();
  },
  ...
  renderList(reFilter = true) {
    ...
    pageItems.forEach(i => {
      ...
      // if (currentMode !== 'view') {
      // Allow delete in all modes? No, only View Mode restricts delete usually, but user asked to enable Edit in View.
      // Let's keep Delete hidden in View Mode for now as per original logic, or enable it?
      // Original: if (currentMode !== 'view') ...
      // Let's keep Delete restricted to specific modes for safety, but Edit is now empowered.
      // }
      if (currentMode !== 'view') {
  ...
  async handleEdit(jan) {
    if (currentMode === 'view') {
      // Open Dialog
      const item = await this.dbGet(jan);
      if (!item) return;
      pendingEditJan = jan;
      dom.dlgTitle.textContent = `${item.name} (${jan})`;
      dom.dlgShelf.value = item.shelf || 0;
      dom.dlgKyakuchu.value = item.kyakuchu || 0;
      dom.dlgPrefab.value = item.prefab || 0;
  ...
  async saveViewEdit() {
    if (!pendingEditJan) return;
    const jan = pendingEditJan;
    const item = await this.dbGet(jan);

    // Update Local
    item.shelf = parseInt(dom.dlgShelf.value) || 0;
    item.kyakuchu = parseInt(dom.dlgKyakuchu.value) || 0;
    item.prefab = parseInt(dom.dlgPrefab.value) || 0;
    item.updatedAt = new Date().toISOString();
    await this.dbPut(item);

    dom.dialog.classList.add('hidden');
    this.renderList();
    this.showStatus('syncing', '全ヶ所同期中...');

    // Sync to all 3 sheets
    try {
      const targets = ['shelf', 'kyakuchu', 'prefab'];
      const values = [item.shelf, item.kyakuchu, item.prefab];

      // Sequential Promise to simplify status
      for (let i = 0; i < 3; i++) {
        const payload = {
          jan: item.jan, name: item.name, makerCode: item.makerCode,
          stock: values[i], updatedAt: item.updatedAt
        };
        await fetch(GAS_API_URL, {
          method: 'POST',
          body: JSON.stringify({ target: targets[i], data: [payload] })
        });
      }
      this.toast('全件保存完了');
      this.showStatus('hidden');
    } catch (e) {
      this.showStatus('error', '一部送信失敗');
      this.toast('同期エラー');
    }
  },
      ...
      Quagga.init({
        inputStream: {
          ...
          constraints: {
            width: { ideal: 640 },
            height: { ideal: 480 },
            facingMode: "environment" // Start SIMPLE. No advanced constraints here.
          }
        },
        ...
        if (err) {
          debugLog(`Init Error: ${err.name} - ${err.message}`);
          console.error(err);
          // Fallback: try without resolutions if Overconstrained
          if (err.name === 'OverconstrainedError') {
            debugLog('Retrying without resolution constraints...');
            // Logic to retry could go here, but let's just warn for now.
          }
          return;
        }

        debugLog('Initialization success. Starting...');
        Quagga.start();
        isScanning = true;

        // Try to enforce autofocus via track capabilities AFTER start
        try {
          const track = Quagga.CameraAccess.getActiveTrack();
          if (track) {
            const caps = track.getCapabilities ? track.getCapabilities() : {};
            debugLog(`Camera Caps: Focus=${caps.focusMode || 'N/A'}`);

            if (caps.focusMode && caps.focusMode.includes('continuous')) {
              debugLog('Applying continuous focus...');
              track.applyConstraints({ advanced: [{ focusMode: 'continuous' }] })
                .then(() => debugLog('Focus applied.'))
                .catch(e => debugLog(`Focus failed: ${e}`));
            } else {
              debugLog('Continuous focus not supported.');
            }
          } else {
            debugLog('No active track found.');
          }
        } catch (e) {
          debugLog(`Focus logic error: ${e}`);
        }
      });
  ...
  async forceUpdate() {
    // Toggle Debug Console first for visibility
    const con = document.getElementById('debug-console');
    if (con.style.display === 'none') {
      con.style.display = 'block';
      if (!confirm('デバッグモードを表示しました。\n更新も行いますか？')) return;
    }

    if (!confirm('最新版を再取得しますか？')) return;
