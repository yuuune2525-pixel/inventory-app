...
const app = {
  ...
  stopScanner() {
    if (isScanning) {
      ...
      if (dom.btnScan) dom.btnScan.textContent = '📷 読取開始';
    }
  },
  ...
  startScanner() {
    ...
    if (dom.btnScan) dom.btnScan.textContent = '⏹ 停止';
    ...
    Quagga.init({
      inputStream: {
        ...
        constraints: {
          width: { ideal: 640 },
          height: { ideal: 480 },
          facingMode: "environment" // Start SIMPLE.
        }
      },
      ...
      try {
        ...
        if (track) {
          ...
          debugLog(`Caps: Focus=${caps.focusMode || 'N/A'}`);
    ...
    Quagga.onDetected(res => {
      if (!isScanning) return; // Prevent extra reads logic
